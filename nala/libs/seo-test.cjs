...
async function runSeoChecks({ page, feature, options = {} }) {
  const issues = [];
  ...
  if (!title || title.trim().length === 0) {
    ...
    if (title.length < config.minTitleLength) {
      issues.push(
        `<title> too short (${title.length} chars, min ${config.minTitleLength})`,
      );
    }
    if (title.length > config.maxTitleLength) {
      issues.push(
        `<title> too long (${title.length} chars, max ${config.maxTitleLength})`,
      );
    }
  }
  ...
  // Images should have alt attribute (alt="" is valid for decorative images)
