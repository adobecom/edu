...
function createBlock(el, cfg, a11yActionUpdate) {
  ...
  cfg.icon = cfg.icon || '/edu/img/icons/aichat.svg';
...
export default async function init(element) {
  const { createTag: createTagfn } = await import(`${libs}/utils/utils.js`);
  const { processTrackingLabels: processTrackingLabelsfn } = await import(`${libs}/martech/attributes.js`);
