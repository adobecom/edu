...
try {
  ...
  ({ getBranchUrl } = require('../libs/constants.cjs'));
} catch {
  ({ DEFAULT_REPO: PROJECT, DEFAULT_ORG: ORG, BASE_URLS, getBranchUrl } = require('../libs/constants.cjs'));
}
...
function parseArgs(args) {
  ...
  if (!parsedParams.project) {
    parsedParams.project = `${parsedParams.repo}-live-${parsedParams.browser}`;
  }

  return parsedParams;
}

function getLocalTestLiveUrl(env, milolibs, repo = PROJECT, owner = ORG) {
